...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var resultArrayGauge = metadata.filter(metadataObj => metadataObj.id == sample);
    ...
    var washing_freq = resultGauge.wfreq
    ...
    //var ytickssorted = otu_ids.reverse();
    var yticks = otu_ids.slice(0, 10).map(i => `OTU ${i} `).reverse();
    // yticks = yticks.map(i => `OTU ` + i)
    console.log(yticks)
    ...
    var gaugeLayout = {
      width: 450, height: 300, margin: { t: -25, b: 0 },
      font: {
        family: "Times New Roman",
        size: 18,
        color: '000000'
